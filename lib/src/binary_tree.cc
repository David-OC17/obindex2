...
BinaryTree::BinaryTree(BinaryDescriptorSetPtr dset, const unsigned tree_id,
                       const unsigned k, const unsigned s)
    : dset_(dset),
      tree_id_(tree_id),
      root_(nullptr),
      k_(k),
...
BinaryTree::~BinaryTree() { deleteTree(); }
...
void BinaryTree::buildNode(BinaryDescriptorSet dset, BinaryTreeNodePtr root) {
  ...
  if (dset.size() < s_) {
    ...
    for (auto it = dset.begin(); it != dset.end(); it++) {
      ...
      for (unsigned i = 0; i < k_; i++) {
        double dist =
            obindex2::BinaryDescriptor::distHamming(*d, *(new_centers[i]));
    ...
    for (unsigned i = 0; i < k_; i++) {
      BinaryTreeNodePtr node =
          std::make_shared<BinaryTreeNode>(false, new_centers[i], root);
...
unsigned BinaryTree::traverseFromRoot(BinaryDescriptorPtr q, NodeQueuePtr pq,
...
void BinaryTree::traverseFromNode(BinaryDescriptorPtr q, BinaryTreeNodePtr n,
...
void BinaryTree::printTree() { printNode(root_); }
...
void BinaryTree::printNode(BinaryTreeNodePtr n) {
  ...
  if (n->isLeaf()) {
    std::cout << "Children descriptors: " << n->childDescriptorSize()
              << std::endl;
  } else {
