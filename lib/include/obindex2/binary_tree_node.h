...
class BinaryTreeNode {
 public:
  // Constructors
  BinaryTreeNode();
  explicit BinaryTreeNode(const bool leaf, BinaryDescriptorPtr desc = nullptr,
                          BinaryTreeNodePtr root = nullptr);

  // Methods
  inline bool isLeaf() { return is_leaf_; }

  inline void setLeaf(const bool leaf) { is_leaf_ = leaf; }

  inline bool isBad() { return is_bad_; }

  inline void setBad(const bool bad) { is_bad_ = bad; }

  inline BinaryDescriptorPtr getDescriptor() { return desc_; }

  inline void setDescriptor(BinaryDescriptorPtr desc) { desc_ = desc; }

  inline BinaryTreeNodePtr getRoot() { return root_; }

  inline void setRoot(BinaryTreeNodePtr root) { root_ = root; }

  inline double distance(BinaryDescriptorPtr desc) {
    return obindex2::BinaryDescriptor::distHamming(*desc_, *desc);
  }

  inline void addChildNode(BinaryTreeNodePtr child) { ch_nodes_.insert(child); }
  ...
  inline unsigned childNodeSize() { return ch_nodes_.size(); }
  ...
  inline unsigned childDescriptorSize() { return ch_descs_.size(); }
